...
for i in range(200):
    ...
    target_rewards = cme_estimator.estimate(target_feature_vec)
    target_reward = target_rewards.sum()
    ...
    loss, theta, logits = policy_grad.train_step(sample_users, null_actions, target_rewards)

    if i % 20 == 0:
        print("iter {}, Expected reward: {}".format(i, expected_reward))
        print("iter {}, CME reward: {}".format(i, target_reward))
        print("iter {}, loss: {}".format(i, target_reward))
        print("iter {}, theta: {}".format(i, theta))
