...
def run_iteration(user_item_vectors, true_weights, null_policy_weight, n_observation, num_iter, est='CME'):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        if est == 'Direct':
            ...
            estimator = wIPS(null_action_probs[:,null_actions], null_rewards)
        elif est == 'DR':
            estimator = DR(null_feature_vec, null_action_probs[:,null_actions], null_rewards)
        elif est == 'CME':
        ...
        for i in range(num_iter):
            ...
            if est == 'Direct':
                ...
                target_reward_vec = estimator.estimate(target_action_probs[:,target_actions])
            elif est == 'DR':
                target_reward_vec = estimator.estimate(target_feature_vec, target_action_probs[:,target_actions])
            elif est == 'CME':
                target_reward_vec = estimator.estimate(target_feature_vec)
