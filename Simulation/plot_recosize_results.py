...
df_list = [pd.read_csv(filename) for filename in glob.glob(os.path.join('jobs/recommendation_size_report/results','*.csv'))]

prelim_result = pd.concat(df_list)
prelim_result.sort_values(by=['n_reco'], ascending=True)

prelim_result = prelim_result[[c for c in prelim_result.columns if 'error' in c] + ['n_reco']]
...
for cond, cond_df in prelim_result.groupby("n_reco"):
...
final_df = pd.melt(winsorized_df, id_vars=["n_reco"], var_name="estimator", value_name="MSE")

# plot the results
ax = sns.pointplot(x="n_reco", y="MSE", hue="estimator", data=final_df,
                   linestyles=["-", "--","-.",":",":"], markers=['o','v','x','^','+'])

ax.set_yscale('log')
ax.set_ylabel("Mean Square Error (MSE)")
ax.set_xlabel('Number of Recommended Items')

xticks = ['%0.1f' % x for x in np.unique(final_df['n_reco'])]
...
plt.savefig('plots/recommendation_size_result.pdf')